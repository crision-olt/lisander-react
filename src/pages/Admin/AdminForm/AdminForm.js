...
function AdminForm(props) {
    ...
    useEffect(() => {
        getUserTootsApi(params.id, 1).then(response => {
            setToots(response);
        }).catch(()=>{
            setToots([]);
        });
    ...
    return (
        <BasicLayout className="user" setRefreshCheckLogin={setRefreshCheckLogin}>
            <div className="w-full  p-1 grid grid-flow-row bg-blue-50 m-auto" style={{maxWidth:600,}}>
            <div className="user__title">
                <h2>{user ? `${user.name} ${user.surnames}` : "Este usuario no existe"}</h2>
            </div>
